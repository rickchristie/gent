...
// -----------------------------------------------------------------------------
// Common Diff Event (User-Defined State Changes)
// -----------------------------------------------------------------------------

// CommonDiffEvent is for tracking state changes with automatic diff generation.
//
// Use ExecutionContext.PublishCommonDiffEvent() to publish state change events:
//
//	ctx.PublishCommonDiffEvent("myapp:config_change", oldConfig, newConfig)
//
// The Diff field is auto-generated by JSON-marshaling both before and after values,
// then computing a unified diff. This is useful for hook implementers who want to
// log or react to state changes.
type CommonDiffEvent struct {
	BaseEvent
	...
	// Diff is the unified diff between JSON-marshaled Before and After.
	// If marshaling fails, contains error message(s) prefixed with "<marshal error: ...>".
	Diff string
}
