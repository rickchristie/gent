...
echo ""
echo "--- Whitelisted Domains (AI Assistants) ---"
...
echo ""
echo "--- Whitelisted Domains (LLM APIs) ---"
...
curl -so /dev/null -w "%{http_code}" -x http://host.docker.internal:3128 https://generativelanguage.googleapis.com 2>/dev/null | grep -qE "^[2-4][0-9][0-9]$" && echo "OK: Google Gemini API" || echo "FAIL: Google Gemini API"
curl -so /dev/null -w "%{http_code}" -x http://host.docker.internal:3128 https://api.x.ai 2>/dev/null | grep -qE "^[2-4][0-9][0-9]$" && echo "OK: xAI API" || echo "FAIL: xAI API"
curl -so /dev/null -w "%{http_code}" -x http://host.docker.internal:3128 https://models.github.ai 2>/dev/null | grep -qE "^[2-4][0-9][0-9]$" && echo "OK: GitHub Models API" || echo "FAIL: GitHub Models API"
...
# Test ONNX runtime libraries
echo ""
echo "--- ONNX Runtime Libraries ---"
if [ -f /home/user/.gent/lib/libonnxruntime.so.1.24.4 ]; then
    echo "OK: libonnxruntime.so.1.24.4 found"
else
    echo "INFO: libonnxruntime not found (run 'go run ./cmd/gent setup onnx' on host)"
fi
if [ -f /home/user/.gent/lib/libtokenizers.a ]; then
    echo "OK: libtokenizers.a found"
else
    echo "INFO: libtokenizers.a not found (run 'go run ./cmd/gent setup onnx' on host)"
fi
if [ -n "${CGO_LDFLAGS}" ]; then
    echo "OK: CGO_LDFLAGS=${CGO_LDFLAGS}"
else
    echo "INFO: CGO_LDFLAGS not set"
fi
if [ -n "${LD_LIBRARY_PATH}" ]; then
    echo "OK: LD_LIBRARY_PATH=${LD_LIBRARY_PATH}"
else
    echo "INFO: LD_LIBRARY_PATH not set"
fi
...
# Test LLM API keys from .env
echo ""
echo "--- LLM API Keys (.env) ---"
[ -n "${GENT_TEST_OPENAI_KEY}" ] && echo "OK: GENT_TEST_OPENAI_KEY set" || echo "INFO: GENT_TEST_OPENAI_KEY not set"
[ -n "${GENT_TEST_ANTHROPIC_KEY}" ] && echo "OK: GENT_TEST_ANTHROPIC_KEY set" || echo "INFO: GENT_TEST_ANTHROPIC_KEY not set"
[ -n "${GENT_TEST_GEMINI_KEY}" ] && echo "OK: GENT_TEST_GEMINI_KEY set" || echo "INFO: GENT_TEST_GEMINI_KEY not set"
[ -n "${GENT_TEST_XAI_KEY}" ] && echo "OK: GENT_TEST_XAI_KEY set" || echo "INFO: GENT_TEST_XAI_KEY not set"
[ -n "${GENT_TEST_GITHUB_TOKEN}" ] && echo "OK: GENT_TEST_GITHUB_TOKEN set" || echo "INFO: GENT_TEST_GITHUB_TOKEN not set"
