...
# Gent native libraries (ONNX runtime + tokenizer, read-only)
GENT_LIB_DIR="${HOME}/.gent/lib"
if [ -d "${GENT_LIB_DIR}" ]; then
    VOLUME_ARGS="${VOLUME_ARGS} -v ${GENT_LIB_DIR}:/home/user/.gent/lib:ro"
fi
...
# Gent: ONNX runtime and tokenizer library paths
if [ -d "${GENT_LIB_DIR}" ]; then
    ENV_ARGS="${ENV_ARGS} -e CGO_LDFLAGS=-L/home/user/.gent/lib"
    ENV_ARGS="${ENV_ARGS} -e LD_LIBRARY_PATH=/home/user/.gent/lib"
fi
# ============================================================================
